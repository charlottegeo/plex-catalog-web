      ...
      <div className="row no-gutters">
        ...
        <div className="col-md-8">
          <div className="card-body">
            ...
            <p className="card-text text-muted">{episode.summary}</p>
            <div
              className="version-info d-flex align-items-center"
              style={{ gap: '8px' }}
            >
              {episode.versions.map((v, i) => (
                <span key={i} className="badge badge-soft-secondary">
                  {formatResolution(v.videoResolution)}
                </span>
              ))}
              {uniqueSubtitles.length > 0 && (
                <>
                  <SubtitlesIcon /> <span>{uniqueSubtitles.join(', ')}</span>
                </>
              )}
            </div>
          </div>
    ...
    <div className="container mt-4">
      <nav aria-label="breadcrumb">
        <ol className="breadcrumb">
          ...
          {show && (
            <li className="breadcrumb-item">
              <Link to={`/media/${encodeURIComponent(show.guid)}`}>
                {show.title}
              </Link>
            </li>
          )}
